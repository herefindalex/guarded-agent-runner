...
| Live mutation | Hard-disabled; the exact read-only tuple has G-01 `PASS`, G-02/G-04 `FAIL`, and the remaining mutation gates `NOT_RUN` | Code, lock and local fixture inspection |
...
| Backup observations | Explicitly unavailable until the M4 offline backup engine exists | Stable reason code; not implemented |
